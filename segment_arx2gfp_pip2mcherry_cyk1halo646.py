...
def organize_3_channels_files(input_dir, ch1_trgt_name, ch2_trgt_name, ch3_trgt_name, file_extension='.tif', prefix_channel='ch1'):
    ...
    for f in input_files_l:
        ...
        if file_extension in f:
            ...
            elif ch3_trgt_name in f:
                # print("CH3", tc)
                organized_dict_out[ch3_trgt_name] = f_ile
                if prefix_channel=='ch3':
                    organized_dict_out['prefix_name'] = f[:f.index(ch3_trgt_name)] + "_"
            else:
...
def segment_arx2_enriched_domains(raw_timecourse_arx2, embryo_mask_timecourse_1, hyst_filt_low_percentil_e_1, hyst_filt_high_percentil_e_1,
                                    hyst_filt_low_stdfacto_r_1=None, hyst_filt_high_stdfacto_r_1=None,
                                    output_low_va_l_1=0, output_high_va_l_1=255, output_d_typ_e_1=np.uint8, iteration_axi_s_1=0):
    """
    this function only supports 3D arrays
    """

    #Copy the input timecourses
    raw_timecourse_arx2_copy = raw_timecourse_arx2.copy()
    embryo_mask_timecourse_1_copy = embryo_mask_timecourse_1.copy()

    #Preprocess the raw timecourse using median filtering
    if iteration_axi_s_1==0:
        median_filt_timecourse_arx2_list = [median_blur_image(raw_timecourse_arx2_copy[a,...]) for a in range(raw_timecourse_arx2_copy.shape[0])]
    elif iteration_axi_s_1==1:
        median_filt_timecourse_arx2_list = [median_blur_image(raw_timecourse_arx2_copy[:,b,:]) for b in range(raw_timecourse_arx2_copy.shape[1])]
    elif iteration_axi_s_1==2:
        median_filt_timecourse_arx2_list = [median_blur_image(raw_timecourse_arx2_copy[...,c]) for c in range(raw_timecourse_arx2_copy.shape[2])]
    
    median_filt_timecourse_arx2 = np.asarray(median_filt_timecourse_arx2_list)

    #Apply hysteresis-based segmentation on the timecourse - NOTE: the function is applied to the raw timecourse while using the median-filtered timecourse for thresholds' calculation
    hysteresis_segmented_timecourse_1 = hysteresis_segmentation_over_axis(raw_timecourse_arx2_copy,
                                                                            hyst_filt_low_percentile=hyst_filt_low_percentil_e_1,
                                                                            hyst_filt_high_percentile=hyst_filt_high_percentil_e_1,
                                                                            hyst_filt_low_stdfactor=hyst_filt_low_stdfacto_r_1,
                                                                            hyst_filt_high_stdfactor=hyst_filt_high_stdfacto_r_1,
                                                                            filtering_logic='strict',
                                                                            roi_mas_k=embryo_mask_timecourse_1_copy,
                                                                            img__4__histogram=median_filt_timecourse_arx2,
                                                                            output_low_val=output_low_va_l_1,
                                                                            output_high_val=output_high_va_l_1,
                                                                            output_d_type=output_d_typ_e_1,
                                                                            iteration_axis=0)
    
    return hysteresis_segmented_timecourse_1
